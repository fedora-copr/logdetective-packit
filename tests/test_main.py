...
def test_publish_message(mock_env_vars, mocker, mock_external_calls):
...
def test_publish_message_exceptions(
...
async def test_call_log_detective_request_exception(
    ...
    build_info = BuildInfo(**MINIMAL_BUILD_INFO)
    mock_external_calls["mock_requests_post"].side_effect = HTTPError
    with pytest.raises(HTTPError):
        await call_log_detective(build_info=build_info)
        mock_server_logger["mock_logger"].assert_called_once()
        mock_external_calls["mock_requests_post"].assert_called_once()


def test_analyze_build_skeleton(
    monkeypatch, mocker, mock_env_vars, mock_external_calls
):
    ...
    mock_external_calls["mock_requests_post"].return_value = mock_response
    ...
    mock_external_calls["mock_requests_post"].assert_called_once_with(
